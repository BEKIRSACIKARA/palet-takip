...
def get_musteri_stoklari(current_user):
    ...
    cursor.execute("""
        SELECT m.id, m.musteri_kodu, m.musteri_adi, m.tabela_adi, 
               pt.id as palet_id, pt.stok_kodu, pt.palet_adi, COALESCE(s.miktar, 0) as miktar
        FROM musteriler m
        CROSS JOIN palet_tipleri pt
        LEFT JOIN stoklar s ON s.stok_sahibi_tip = 'MUSTERI' AND s.stok_sahibi_id = m.id AND s.palet_tipi_id = pt.id
        WHERE COALESCE(s.miktar, 0) > 0
        ORDER BY m.musteri_adi, pt.id
    """)
    sonuc = cursor.fetchall()
    cursor.close()
    conn.close()
