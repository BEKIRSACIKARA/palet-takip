...
def veritabani_olustur():
    ...
    # --- YENÄ° EKLENECEK KISIM BAÅLANGICI ---
    # Eksik olan makbuz_no sÃ¼tununu tabloya eklemeye zorluyoruz.
    try:
        cursor.execute("ALTER TABLE hareketler ADD COLUMN makbuz_no TEXT")
        conn.commit()
    except Exception:
        # SÃ¼tun zaten varsa (veya eklendiyse) hata verir, veritabanÄ±nÄ±n kilitlenmemesi iÃ§in iÅlemi geri alÄ±yoruz (rollback)
        conn.rollback()
    # --- YENÄ° EKLENECEK KISIM BÄ°TÄ°ÅÄ° ---
...
def stok_guncelle(stok_sahibi_tip, stok_sahibi_id, palet_tipi_id, degisim):
    ...
    if mevcut:
        ...
        if degisim < 0:
            cursor.close()
            conn.close()
            return False, "Stok kaydÄ± bulunamadÄ±!"
...
def makbuz_no_olustur():
    ...
    cursor = conn.cursor()
    try:
        # Metin parÃ§alamak yerine direkt en yÃ¼ksek ID'yi buluyoruz. 
        # Bu yÃ¶ntem formatÄ± bozuk test verilerinden asla etkilenmez.
...
def login():
    ...
    return jsonify({'success': False, 'hata': 'HatalÄ± kullanÄ±cÄ± adÄ± veya Åifre'}), 401
...
def get_kullanici_listesi(current_user):
    if current_user['tip'] != 'DEPOCU':
        return jsonify({'hata': 'Yetkisiz eriÅim'}), 403
    conn = get_db_connection()
    cursor = conn.cursor()
...
def dagitici_ekle(current_user):
    ...
    if not kullanici_adi or not ad_soyad or not sifre:
        return jsonify({'hata': 'TÃ¼m alanlar gerekli'}), 400
    if len(sifre) < 4:
        return jsonify({'hata': 'Åifre en az 4 karakter olmalÄ±'}), 400
    conn = get_db_connection()
    cursor = conn.cursor()
    try:
        ...
        cursor.close()
        conn.close()
        return jsonify({'success': True, 'id': dagitici_id, 'mesaj': 'DaÄÄ±tÄ±cÄ± eklendi'})
...
def musteri_ekle(current_user):
    ...
    try:
        ...
        cursor.close()
        conn.close()
        return jsonify({'success': True, 'id': musteri_id, 'mesaj': 'MÃ¼Återi eklendi'})
    except Exception as e:
        cursor.close()
        conn.close()
        return jsonify({'hata': str(e)}), 400
...
def get_musteri_stoklari(current_user):
    ...
    # Sadece elinde 0'dan fazla palet olan (stoklu) mÃ¼Återileri ve palet tiplerini Ã§ekiyoruz
    cursor.execute('''
    ...
    # ArayÃ¼zÃ¼n beklediÄi formata (JSON) uygun hale getiriyoruz
    musteriler = {}
    for row in sonuc:
        m_id = row[0]
...
def transfer_yap(current_user):
    ...
    if miktar <= 0:
        return jsonify({'hata': 'Miktar pozitif olmalÄ±'}), 400
    ...
    if not palet:
        cursor.close()
        conn.close()
        return jsonify({'hata': 'GeÃ§ersiz palet tipi'}), 400
    ...
    if kullanici_tip == 'DEPOCU':
        if hareket_tipi == 'DEPO_DAGITICI':
            ...
            if not dagitici:
                cursor.close()
                conn.close()
                return jsonify({'hata': 'GeÃ§ersiz daÄÄ±tÄ±cÄ± ID'}), 400
            alan_adi = f"{dagitici[0]} ({dagitici[1]})"
            aciklama = f"{palet[2]} - {miktar} adet {dagitici[0]} daÄÄ±tÄ±cÄ±sÄ±na transfer edildi"
            ...
            aciklama = f"{palet[2]} - {miktar} adet {dagitici[0]} daÄÄ±tÄ±cÄ±sÄ±ndan iade alÄ±ndÄ±"
        else:
            cursor.close()
            conn.close()
            return jsonify({'hata': 'GeÃ§ersiz hareket tipi'}), 400
        ...
        if hareket_tipi == 'DAGITICI_MUSTERI':
            ...
            if not musteri:
                cursor.close()
                conn.close()
                return jsonify({'hata': 'GeÃ§ersiz mÃ¼Återi ID'}), 400
            alan_adi = f"{musteri[0]} - {musteri[1]}"
            aciklama = f"{palet[2]} - {miktar} adet {musteri[1]} mÃ¼Återisine verildi"
            ...
            aciklama = f"{palet[2]} - {miktar} adet {musteri[1]} mÃ¼Återisinden iade alÄ±ndÄ±"
        else:
            cursor.close()
            conn.close()
            return jsonify({'hata': 'GeÃ§ersiz hareket tipi'}), 400
        ...
        cursor.close()
        conn.close()
        return jsonify({'hata': 'Yetkisiz kullanÄ±cÄ±'}), 403
    ...
    cursor.close()
    conn.close()
    return jsonify({'success': True, 'mesaj': f'Transfer baÅarÄ±lÄ±! {miktar} adet {palet[2]} transfer edildi.', 'makbuz_no': makbuz_no})
...
def makbuz_goster(current_user, makbuz_no): # <-- Sadece current_user eklendi
    ...
    if not makbuz:
        cursor.close()
        conn.close()
        return jsonify({'hata': 'Makbuz bulunamadÄ±'}), 404
...
def makbuz_pdf(current_user, makbuz_no):
    ...
    font_path = os.path.join(os.path.dirname(__file__), 'arial.ttf') # Dosya adÄ±nÄ±n arial.ttf olduÄundan emin ol
    try:
    ...
    # Stilleri TÃ¼rkÃ§e font ile eziyoruz
    ...
    normal_style = styles['Normal'] # Alt satÄ±ra inmesi gereken uzun metinler iÃ§in stili hazÄ±rladÄ±k
    
    story = []
    story.append(Paragraph("ULUDAÄ Ä°ÃECEK", title_style))
    story.append(Paragraph("KONYA BÃLGE DEPO", title_style))
    story.append(Paragraph("PALET Ä°ÅLEM MAKBUZU", title_style))
    story.append(Spacer(1, 20))
    
    tip_text = {'DEPO_DAGITICI': 'Depo â DaÄÄ±tÄ±cÄ±', 'DAGITICI_MUSTERI': 'DaÄÄ±tÄ±cÄ± â MÃ¼Återi', 'MUSTERI_DAGITICI': 'MÃ¼Återi â DaÄÄ±tÄ±cÄ±', 'DAGITICI_DEPO': 'DaÄÄ±tÄ±cÄ± â Depo'}.get(makbuz[3], makbuz[3])
    
    # --- YANA TAÅMAYI ENGELLEYEN KISIM ---
    # Uzun olabilecek metinleri Paragraph() iÃ§ine aldÄ±k ki sÄ±nÄ±rda alt satÄ±ra insin
    data = [
        ['Makbuz No:', makbuz[1]], 
        ['Tarih:', makbuz[2]], 
        ['Ä°Ålem TÃ¼rÃ¼:', tip_text], 
        ['Teslim Eden:', Paragraph(makbuz[6], normal_style)], 
        ['Teslim Alan:', Paragraph(makbuz[9], normal_style)], 
        ['Toplam Miktar:', str(makbuz[10])], 
        ['AÃ§Ä±klama:', Paragraph(makbuz[11] or "", normal_style)], 
        ['Ä°Ålemi Yapan:', makbuz[13]]
    ]
    ...
    table.setStyle(TableStyle([
        ...
        ('VALIGN', (0, 0), (-1, -1), 'MIDDLE') # Metinler alt satÄ±ra inince hÃ¼crede ortalansÄ±n
    ]))
    story.append(table)
    
    story.append(Spacer(1, 20))
    
    # Detaylar tablosunda da palet adÄ± uzun olabilir diye Ã¶nlem alÄ±yoruz
    detay_data = [['Stok Kodu', 'Palet AdÄ±', 'Miktar']] + [[d[0], Paragraph(d[1], normal_style), str(d[2])] for d in detaylar]
    ...
    story.append(Paragraph("Yetkili Ä°mza", normal_style))
...
def get_hareketler(current_user):
    ...
    hareketler = []
    for h in sonuc:
        tip_text = {'DEPO_DAGITICI': 'DepoâDaÄÄ±tÄ±cÄ±', 'DAGITICI_MUSTERI': 'DaÄÄ±tÄ±cÄ±âMÃ¼Återi', 'MUSTERI_DAGITICI': 'MÃ¼ÅteriâDaÄÄ±tÄ±cÄ±', 'DAGITICI_DEPO': 'DaÄÄ±tÄ±cÄ±âDepo', 'DEPO_STOK_HAREKET': 'Depo Stok Hareketi'}.get(h[3], h[3])
...
def depo_stok_hareket(current_user):
    ...
    if islem_tipi == 'azalt':
        ...
        hareket_aciklama = f"STOK AZALTMA: {miktar} adet {palet[1]} azaltÄ±ldÄ±. Sebep: {aciklama}"
    else:
        yeni_miktar, degisim = mevcut + miktar, +miktar
    ...
    cursor.close()
    conn.close()
    return jsonify({'success': True, 'mesaj': f'{palet[1]} stoÄu gÃ¼ncellendi. Yeni miktar: {yeni_miktar}', 'yeni_miktar': yeni_miktar})
...
def rapor_export(current_user):
    ...
    if rapor_tipi == 'hareketler':
        ...
        headers = ['Tarih', 'Yapan KullanÄ±cÄ±', 'Ä°Ålem Tipi', 'Stok Kodu', 'Palet AdÄ±', 'Miktar', 'Makbuz No', 'AÃ§Ä±klama']
        ...
        for row_idx, row in enumerate(cursor.fetchall(), 2):
            tip_text = {'DEPO_DAGITICI': 'DepoâDaÄÄ±tÄ±cÄ±', 'DAGITICI_MUSTERI': 'DaÄÄ±tÄ±cÄ±âMÃ¼Återi', 'MUSTERI_DAGITICI': 'MÃ¼ÅteriâDaÄÄ±tÄ±cÄ±', 'DAGITICI_DEPO': 'DaÄÄ±tÄ±cÄ±âDepo', 'DEPO_STOK_HAREKET': 'Depo Stok Hareketi'}.get(row[2], row[2])
        ...
        sheet = workbook.active
        sheet.title = "Stoklar"
        headers = ['Stok Sahibi', 'Stok Kodu', 'Palet AdÄ±', 'Miktar']
        for col, header in enumerate(headers, 1):
            cell = sheet.cell(row=1, column=col, value=header)
            cell.font = Font(bold=True, color="FFFFFF")
            cell.fill = PatternFill(start_color="2196F3", end_color="2196F3", fill_type="solid")
        ...
        sheet = workbook.active
        sheet.title = "MÃ¼Återiler"
        headers = ['MÃ¼Återi Kodu', 'MÃ¼Återi AdÄ±', 'Tabela AdÄ±']
        for col, header in enumerate(headers, 1):
            cell = sheet.cell(row=1, column=col, value=header)
            cell.font = Font(bold=True, color="FFFFFF")
            cell.fill = PatternFill(start_color="2196F3", end_color="2196F3", fill_type="solid")
...
def rapor_pdf(current_user):
    ...
    story.append(Paragraph("ULUDAÄ Ä°ÃECEK KONYA BÃLGE DEPO", title_style))
    story.append(Paragraph(f"{rapor_tipi.upper()} RAPORU", title_style))
    story.append(Spacer(1, 10))
    if baslangic and bitis:
        date_text = f"Tarih AralÄ±ÄÄ±: {baslangic} - {bitis}"
    else:
        date_text = f"OluÅturma Tarihi: {datetime.datetime.now().strftime('%d.%m.%Y %H:%M')}"
    ...
    if rapor_tipi == 'hareketler':
        ...
        sonuc = cursor.fetchall()
        data = [['Tarih', 'Yapan', 'Ä°Ålem Tipi', 'Stok Kodu', 'Palet AdÄ±', 'Miktar', 'Makbuz No', 'AÃ§Ä±klama']]
        for row in sonuc:
            tip_text = {'DEPO_DAGITICI': 'DepoâDaÄÄ±tÄ±cÄ±', 'DAGITICI_MUSTERI': 'DaÄÄ±tÄ±cÄ±âMÃ¼Återi', 'MUSTERI_DAGITICI': 'MÃ¼ÅteriâDaÄÄ±tÄ±cÄ±', 'DAGITICI_DEPO': 'DaÄÄ±tÄ±cÄ±âDepo', 'DEPO_STOK_HAREKET': 'Depo Stok Hareketi'}.get(row[2], row[2])
        ...
        story.append(Paragraph("ð¦ DEPO STOÄU", styles['Heading2']))
        data = [['Stok Kodu', 'Palet AdÄ±', 'Miktar']] + [[r[0], r[1], str(r[2])] for r in depo_stok]
        ...
        story.append(Paragraph("ð DAÄITICI STOKLARI", styles['Heading2']))
        data = [['DaÄÄ±tÄ±cÄ±', 'Stok Kodu', 'Palet AdÄ±', 'Miktar']] + [[r[0], r[1], r[2], str(r[3])] for r in dagitici_stok]
        ...
        story.append(Paragraph("ðª MÃÅTERÄ° STOKLARI", styles['Heading2']))
        data = [['MÃ¼Återi', 'Stok Kodu', 'Palet AdÄ±', 'Miktar']] + [[f"{r[0]} - {r[1]}", r[2], r[3], str(r[4])] for r in musteri_stok]
        ...
        story.append(Paragraph("ð EN ÃOK TRANSFER YAPAN DAÄITICILAR", styles['Heading2']))
        data = [['DaÄÄ±tÄ±cÄ±', 'Transfer SayÄ±sÄ±']] + [[r[0], str(r[1])] for r in en_cok]
        ...
        story.append(Paragraph("ð¦ EN ÃOK KULLANILAN PALET TÄ°PLERÄ°", styles['Heading2']))
        data = [['Palet Tipi', 'KullanÄ±m SayÄ±sÄ±', 'Toplam Miktar']] + [[r[0], str(r[1]), str(r[2])] for r in palet_kullanim]
...
def musteri_excel_yukle(current_user):
    ...
    if 'file' not in request.files:
        return jsonify({'hata': 'Dosya bulunamadÄ±'}), 400
    ...
    try:
        ...
        return jsonify({'hata': f'Excel okunamadÄ±: {str(e)}'}), 400
    headers = [str(sheet.cell(row=1, column=col).value or '').strip().upper() for col in range(1, sheet.max_column + 1)]
    required_columns = ['MÃÅTERÄ° KODU', 'MÃÅTERÄ° ADI', 'TABELA ADI']
    for col in required_columns:
        if col not in headers:
            return jsonify({'hata': f"'{col}' sÃ¼tunu bulunamadÄ±"}), 400
    ...
    for row in range(2, sheet.max_row + 1):
        musteri_kodu = str(sheet.cell(row=row, column=col_indices['MÃÅTERÄ° KODU']).value or '').strip()
        musteri_adi = str(sheet.cell(row=row, column=col_indices['MÃÅTERÄ° ADI']).value or '').strip()
        tabela_adi = str(sheet.cell(row=row, column=col_indices['TABELA ADI']).value or '').strip()
        if not musteri_kodu or not musteri_adi or not tabela_adi:
            hatalar.append(f"SatÄ±r {row}: BoÅ alan var")
            continue
        if musteri_kodu in mevcut_musteriler:
            try:
                ...
                hatalar.append(f"SatÄ±r {row}: GÃ¼ncelleme hatasÄ± - {str(e)}")
        else:
            try:
                cursor.execute("INSERT INTO musteriler (musteri_kodu, musteri_adi, tabela_adi) VALUES (%s, %s, %s) RETURNING id", (musteri_kodu, musteri_adi, tabela_adi))
                musteri_id = cursor.fetchone()[0]
                ...
                hatalar.append(f"SatÄ±r {row}: Ekleme hatasÄ± - {str(e)}")
    conn.commit()
    cursor.close()
    conn.close()
    return jsonify({'success': True, 'eklenen': eklenen, 'guncellenen': guncellenen, 'hatalar': hatalar, 'mesaj': f'{eklenen} yeni, {guncellenen} gÃ¼ncellendi.'})
...
def yedekle(current_user):
    ...
    with zipfile.ZipFile(buffer, 'w', zipfile.ZIP_DEFLATED) as zipf:
        ...
        info = f"Palet Takip Sistemi YedeÄi\nOluÅturma: {datetime.datetime.now().strftime('%d.%m.%Y %H:%M:%S')}\nOluÅturan: {current_user['ad_soyad']}"
...
def yedekleme_ayarla(current_user):
    ...
    cursor.close()
    conn.close()
    return jsonify({'success': True, 'mesaj': f'Otomatik yedekleme {"aÃ§Ä±ldÄ±" if aktif else "kapatÄ±ldÄ±"}. Periyot: {periyot}, Saat: {saat}'})
...
def hareketleri_sifirla(current_user):
    # Sadece Ana Depocu bu iÅlemi yapabilir
    if current_user['tip'] != 'DEPOCU':
        return jsonify({'hata': 'Yetkisiz eriÅim'}), 403
    ...
    cursor = conn.cursor()
    try:
        # 1. Makbuz detaylarÄ±nÄ± ve makbuzlarÄ± sil
        cursor.execute("DELETE FROM makbuz_detaylari")
        cursor.execute("DELETE FROM makbuzlar")
        
        # 2. TÃ¼m hareket geÃ§miÅini sil
        cursor.execute("DELETE FROM hareketler")
        
        # 3. Herkesin (Depo, DaÄÄ±tÄ±cÄ±, MÃ¼Återi) stoklarÄ±nÄ± 0'la
        cursor.execute("UPDATE stoklar SET miktar = 0")

        conn.commit()
        return jsonify({'success': True, 'mesaj': 'TÃ¼m hareketler, makbuzlar ve stoklar baÅarÄ±yla sÄ±fÄ±rlandÄ±!'})
    except Exception as e:
        conn.rollback()
...
def stok_excel_yukle(current_user):
    ...
    def std_text(text):
        ...
        return str(text).strip().upper().replace('Ä°', 'I').replace('Å', 'S').replace('Ä', 'G').replace('Ã', 'U').replace('Ã', 'O').replace('Ã', 'C')
    ...
    for col in required_columns:
        if col not in headers:
            return jsonify({'hata': f"'{col}' sÃ¼tunu bulunamadÄ±. LÃ¼tfen dÄ±Åa aktarÄ±lan 'Stoklar' rapor formatÄ±nÄ± bozmadÄ±ÄÄ±nÄ±zdan emin olun."}), 400
    ...
    for row in range(2, sheet.max_row + 1):
        ...
        try:
            ...
            hatalar.append(f"SatÄ±r {row}: Miktar sayÄ± formatÄ±nda deÄil")
            continue
            
        if palet_kodu not in paletler:
            hatalar.append(f"SatÄ±r {row}: GeÃ§ersiz palet kodu ({palet_kodu_raw})")
            continue
        ...
        if sahip_str == 'DEPO':
            ...
            if musteri_kodu not in musteriler:
                hatalar.append(f"SatÄ±r {row}: MÃ¼Återi bulunamadÄ± ({musteri_kodu_raw})")
                continue
            sahip_tipi_db = 'MUSTERI'
            stok_sahibi_id = musteriler[musteri_kodu]
        else: 
            if sahip_str not in dagiticilar:
                hatalar.append(f"SatÄ±r {row}: DaÄÄ±tÄ±cÄ± bulunamadÄ± ({sahip_str_raw})")
                continue
        ...
        try:
            ...
            hatalar.append(f"SatÄ±r {row}: VeritabanÄ± hatasÄ± - {str(e)}")
            
    conn.commit()
    cursor.close()
    conn.close()
    
    return jsonify({'success': True, 'guncellenen': guncellenen, 'hatalar': hatalar, 'mesaj': f'{guncellenen} stok kaydÄ± baÅarÄ±yla eÅitlendi.'})
...
def rapor_dashboard(current_user):
    ...
    # PostgreSQL Ã§Ã¶kmesini engellemek iÃ§in string kÄ±yaslamasÄ± yapÄ±yoruz
    ...
    try:
        ...
        # 3. EN ÃOK BEKLEYEN MÃÅTERÄ°LER
        cursor.execute('''
        ...
        # 4. DAÄITICI PERFORMANSI (HatanÄ±n dÃ¼zeltildiÄi yer burasÄ± -> virgÃ¼l sonrasÄ± eklendi)
        cursor.execute('''
            SELECT u.ad_soyad, 
        ...
        cursor.close()
        conn.close()
        return jsonify({'hata': f'Sistemsel Veri HatasÄ±: {str(e)}'}), 500
...
def toplanacak_paletler(current_user):
    ...
    try:
        # TÃ¼m mÃ¼Återilerdeki mevcut pozitif stoklarÄ± alÄ±yoruz
        cursor.execute('''
        ...
        for m in musteri_stoklari:
            ...
            # Bu mÃ¼Återiye yapÄ±lan palet VERME (DAGITICI -> MUSTERI) iÅlemlerini en yeniden eskiye Ã§ek
            cursor.execute('''
            ...
            # FIFO MantÄ±ÄÄ±: GÃ¼ncel stok miktarÄ±nÄ±, geÃ§miÅteki fiÅlere (sondan baÅa) daÄÄ±tÄ±yoruz
            for h in verme_hareketleri:
                ...
                # DaÄÄ±tÄ±cÄ± giriÅ yaptÄ±ysa, sadece kendi palet bÄ±raktÄ±ÄÄ± yerleri gÃ¶rsÃ¼n
                if current_user['tip'] == 'DAGITICI' and h_dagitici_id == current_user['id']:
                    ilgili_dagitici_mi = True

                # Tarih dÃ¶nÃ¼ÅÃ¼mÃ¼ (PostgreSQL objesi veya String olma ihtimaline karÅÄ± gÃ¼venli)
            ...
            # HiÃ§ hareketi yok ama stoÄu varsa (Ã¶rneÄin Excel'den yÃ¼klendiyse) en eski sepete at
            if kalan_stok > 0:
                yas_22_arti += kalan_stok

            # EÄer Ana Depocu ise herkesi gÃ¶rsÃ¼n, DaÄÄ±tÄ±cÄ± ise sadece kendi bÄ±raktÄ±ÄÄ± mÃ¼Återileri gÃ¶rsÃ¼n
            if current_user['tip'] == 'DEPOCU' or (current_user['tip'] == 'DAGITICI' and ilgili_dagitici_mi):
                # Ekranda kalabalÄ±k yapmamasÄ± iÃ§in sadece 1 haftadan (7 gÃ¼n) daha eski paleti olanlarÄ± listeye al
        ...
        # En riskli olanlarÄ± (22+ ve 15+ olanlarÄ±) en Ã¼ste getirecek Åekilde sÄ±rala
