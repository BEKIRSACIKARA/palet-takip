...
def veritabani_olustur():
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS makbuzlar (
            ...
            toplam_miktar INTEGER NOT NULL,
            aciklama TEXT,
            yapan_kullanici_id INTEGER NOT NULL,
            yapan_kullanici_adi TEXT NOT NULL,
            FOREIGN KEY (yapan_kullanici_id) REFERENCES kullanicilar(id)
        )
    ''')
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS makbuz_detaylari (
            id SERIAL PRIMARY KEY,
            makbuz_id INTEGER NOT NULL,
            palet_tipi_id INTEGER NOT NULL,
            stok_kodu TEXT NOT NULL,
            palet_adi TEXT NOT NULL,
            miktar INTEGER NOT NULL,
            FOREIGN KEY (makbuz_id) REFERENCES makbuzlar(id),
            FOREIGN KEY (palet_tipi_id) REFERENCES palet_tipleri(id)
        )
    ''')
...
def makbuz_kaydet(transfer_data):
    ...
    cursor.execute('''
        INSERT INTO makbuzlar (
            ...
            gonderen_tip, gonderen_id, gonderen_adi,
            alan_tip, alan_id, alan_adi,
            toplam_miktar, aciklama,
            yapan_kullanici_id, yapan_kullanici_adi
        ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
        RETURNING id
    ''', (
        makbuz_no, tarih, transfer_data['islem_tipi'],
        transfer_data['gonderen_tip'], transfer_data['gonderen_id'], transfer_data['gonderen_adi'],
        transfer_data['alan_tip'], transfer_data['alan_id'], transfer_data['alan_adi'],
        transfer_data['toplam_miktar'], transfer_data['aciklama'],
        transfer_data['yapan_id'], transfer_data['yapan_adi']
    ))
    ...
    for detay in transfer_data['detaylar']:
        cursor.execute('''
            INSERT INTO makbuz_detaylari (
                makbuz_id, palet_tipi_id, stok_kodu, palet_adi, miktar
            ) VALUES (%s, %s, %s, %s, %s)
        ''', (
            makbuz_id, detay['palet_tipi_id'], detay['stok_kodu'],
            detay['palet_adi'], detay['miktar']
        ))
...
def transfer_yap(current_user):
    ...
    kullanici_adi = current_user['ad_soyad']
    kullanici_kadi = current_user['kullanici_adi']
    ...
    transfer_data = {
        ...
        'yapan_adi': f"{kullanici_adi} ({kullanici_kadi})",
        'detaylar': [],
    ...
    if kullanici_tip == 'DEPOCU':
        ...
        if hareket_tipi == 'DAGITICI_MUSTERI':
            ...
            alan_adi = f"{kullanici_adi} ({kullanici_kadi})"
    ...
    transfer_data['gonderen_tip'] = gonderen_tip
    transfer_data['gonderen_id'] = gonderen_id
    transfer_data['gonderen_adi'] = gonderen_adi
    transfer_data['alan_tip'] = alan_tip
    transfer_data['alan_id'] = alan_id
    transfer_data['alan_adi'] = alan_adi
    transfer_data['aciklama'] = aciklama
    
    transfer_data['detaylar'].append({
    ...
    return jsonify({
        'success': True,
        'mesaj': f'Transfer başarılı! {miktar} adet {palet[2]} transfer edildi.',
        'makbuz_no': makbuz_no
    })
...
def makbuz_goster(makbuz_no):
    ...
    return jsonify({
        ...
        'tarih': makbuz[2],
        'islem_tipi': makbuz[3],
        'gonderen_adi': makbuz[6],
        'alan_adi': makbuz[9],
        'toplam_miktar': makbuz[10],
        'aciklama': makbuz[11],
        'yapan_adi': makbuz[13],
        'detaylar': [{'stok_kodu': d[0], 'palet_adi': d[1], 'miktar': d[2]} for d in detaylar]
    })
...
def get_hareketler_filtreli(current_user):
    ...
    query = '''
        SELECT h.tarih, u.kullanici_adi, u.ad_soyad, h.hareket_tipi,
               pt.stok_kodu, pt.palet_adi, h.miktar, h.aciklama, h.makbuz_no,
               CASE 
                   WHEN h.gonderen_tip = 'DAGITICI' THEN (SELECT ad_soyad FROM kullanicilar WHERE id = h.gonderen_id)
                   WHEN h.alan_tip = 'DAGITICI' THEN (SELECT ad_soyad FROM kullanicilar WHERE id = h.alan_id)
                   ELSE NULL
               END as ilgili_dagitici
        FROM hareketler h
        JOIN kullanicilar u ON h.yapan_kullanici_id = u.id
        JOIN palet_tipleri pt ON h.palet_tipi_id = pt.id
        WHERE 1=1
...
def rapor_pdf(current_user):
    ...
    if rapor_tipi == 'hareketler':
        ...
        data = [['Stok Kodu', 'Palet Tipi', 'Miktar']] + [[r[0], r[1], str(r[2])] for r in depo_stok]
        ...
        data = [['Dağıtıcı', 'Stok Kodu', 'Palet Tipi', 'Miktar']] + [[r[0], r[1], r[2], str(r[3])] for r in dagitici_stok]
        ...
        data = [['Müşteri', 'Stok Kodu', 'Palet Tipi', 'Miktar']] + [[f"{r[0]} - {r[1]}", r[2], r[3], str(r[4])] for r in musteri_stok]
...
def rapor_export(current_user):
    ...
    if rapor_tipi == 'hareketler':
        ...
        if baslangic and bitis:
            cursor.execute('''
                SELECT h.tarih, u.ad_soyad, h.hareket_tipi, pt.stok_kodu, pt.palet_adi, h.miktar, h.makbuz_no, h.aciklama
                FROM hareketler h
                JOIN kullanicilar u ON h.yapan_kullanici_id = u.id
                JOIN palet_tipleri pt ON h.palet_tipi_id = pt.id
                WHERE DATE(h.tarih) BETWEEN %s AND %s
...
def yedekle(current_user):
    ...
    with zipfile.ZipFile(buffer, 'w', zipfile.ZIP_DEFLATED) as zipf:
        ...
        if kullanicilar:
            csv_buffer = BytesIO()
            csv_writer = csv.writer(csv_buffer)
            csv_writer.writerow(['id', 'kullanici_adi', 'tip', 'ad_soyad'])
...
Toplam Kayıtlar:
- Hareketler: {len(hareketler)}
- Stoklar: {len(stoklar)}
- Müşteriler: {len(musteriler)}
- Kullanıcılar: {len(kullanicilar)}
- Palet Tipleri: {len(palet_tipleri)}
- Makbuzlar: {len(makbuzlar)}
- Makbuz Detayları: {len(makbuz_detaylari)}
"""
    ...
    return send_file(
        ...
        as_attachment=True,
        download_name=f'palet_takip_yedek_{datetime.datetime.now().strftime("%Y%m%d_%H%M%S")}.zip'
    )
...
def yedekleme_ayarla(current_user):
    ...
    periyot = data.get('periyot', 'gunluk')
    saat = data.get('saat', '03:00')
    ...
    return jsonify({
        'success': True,
        'mesaj': f'Otomatik yedekleme {"açıldı" if aktif else "kapatıldı"}. Periyot: {periyot}, Saat: {saat}'
    })
...
def yedekleme_ayarlari(current_user):
    ...
    cursor.execute('SELECT key, value FROM ayarlar WHERE key LIKE %s', ('yedekleme_%',))
    sonuc = cursor.fetchall()
    cursor.close()
    conn.close()
    ...
    return jsonify({
        'aktif': ayarlar.get('aktif', 'False') == 'True',
        'periyot': ayarlar.get('periyot', 'gunluk'),
        'saat': ayarlar.get('saat', '03:00')
    })
